...
import { Pair, Bundle, Token, SwapdexFactory, SwapdexDayData, PairDayData, TokenDayData } from '../types/schema'
import { ONE_BI, ZERO_BD, ZERO_BI, FACTORY_ADDRESS } from './helpers'

export function updateSwapdexDayData(event: EthereumEvent): SwapdexDayData {
  let uniswap = SwapdexFactory.load(FACTORY_ADDRESS)
  let timestamp = event.block.timestamp.toI32()
  let dayID = timestamp / 86400
  let dayStartTimestamp = dayID * 86400
  let uniswapDayData = SwapdexDayData.load(dayID.toString())
  if (uniswapDayData === null) {
    uniswapDayData = new SwapdexDayData(dayID.toString())
  ...
  return uniswapDayData as SwapdexDayData
}
